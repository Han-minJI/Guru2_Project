...
class HabRegPage : AppCompatActivity() {
    lateinit var btnBckToHabLst : ImageButton   // 나의 생활 체크리스트 화면으로 가는 Button

    lateinit var edtHabName : EditText          // 나의 생활 습관 목표 입력하는 EditText
    ...
    lateinit var btnHabReg : ImageButton    // 등록하기 버튼 -> 클릭 시 <HabLstPage.kt - 나의 생활 체크리스트> 화면으로 이동

    var timePickerDialog: TimePickerDialog? = null // 시간 입력 EditText를 클릭하면 나타날 Dialog

    lateinit var dbManager: DBManager    // DBManager 객체
    lateinit var sqlitedb: SQLiteDatabase  // SQLiteDatabase 객체

    // 사용자가 선택한 요일을 저장할 변수 -> 나의 생활 체크리스트 화면에서 오늘 요일에 해당하는 목표 체크리스트만 표시되기 위해 필요
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        edtHabName = findViewById(R.id.edtHabName)            // 나의 생활 체크 목표 입력하는 EditText
        ...
        edtHabCnt = findViewById(R.id.edtHabCnt)    // 나의 생활 체크 목표의 자세한 내용을 입력하는 EditText
        btnHabReg = findViewById(R.id.btnHabReg)    // 등록하기 버튼
        ...
        btnHabReg.setOnClickListener {
            ...
            // HabLstPage (나의 생활(목표) 체크리스트 화면)으로 Intent 전달하며, 해당 화면으로 화면 전환
